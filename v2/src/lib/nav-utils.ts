// Navigation guidance math — ported verbatim from v1 lib/nav-utils.ts.
// Pure functions (unit-tested in nav-utils.test.ts).

import { haversineDistance } from "./units";
...
export function pointToSegmentDistance(
  px: number,
  py: number, // point (lon, lat)
  ax: number,
  ay: number, // segment start (lon, lat)
  bx: number,
  by: number // segment end (lon, lat)
...
export function distanceAlongCoordsFromProjection(
  ...
  const segLen = haversineDistance(
    ...
    coords[segmentIndex][0],
    coords[segmentIndex + 1][1],
    coords[segmentIndex + 1][0]
  );
  let dist = (1 - t) * segLen;
...
export function maneuverInstruction(type: string, modifier?: string, streetName?: string): string {
  ...
  const direction = modifier ? modifier.charAt(0).toUpperCase() + modifier.slice(1) : "";
  ...
  if (type === "roundabout" || type === "rotary")
    return streetName ? `Take roundabout to ${streetName}` : "Enter roundabout";
...
/** HDOP-based multiplier for the off-route threshold. */
...
export function computeOffRouteScore(opts: {
  ...
  if (speed >= 2.0 && distance > 15) {
    ...
    const headingTolerance = distToManeuver < 100 ? 90 + (1 - distToManeuver / 100) * 45 : 90;
