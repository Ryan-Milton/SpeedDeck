...
import { convertDistance, convertSpeed, distanceUnitLabel, formatNavDistance } from '../../lib/utils'
...
export function DashboardNavOverlay(): React.JSX.Element | null {
  ...
  return (
    <>
      ...
      {/* Bottom: ETA + remaining + street name + speed limit */}
        ...
        <div className="rounded-2xl bg-surface-card/95 backdrop-blur-xl px-4 py-2.5 flex items-center gap-3">
          ...
          {speedLimitDisplay !== null && (
            <>
              ...
              <div className="w-8 h-8 rounded-full border-2 border-text-secondary flex items-center justify-center shrink-0">
                <span className="text-xs font-bold text-text-primary tabular-nums">{speedLimitDisplay}</span>
              </div>
            </>
          )}
        </div>
